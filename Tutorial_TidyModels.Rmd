...
Calculando as métricas de erro de predição
```{r}
reg_cv %>%
  collect_metrics()
```
...
Para avaliar o desempenho do modelo através das métricas na validação cruzada:
...
A métrica obtida no conjunto de validação é tomada como uma estimativa da métrica no conjunto de teste.
...
Um gráfico de preditos versus observados, separados por cada *fold* da validação cruzada:
...
Avaliando o desempenho do modelo nas reamostras da validação cruzada:

```{r}
ctrl_abalone <- control_resamples(save_pred = TRUE)
tree_cv <- fit_resamples(tree_wf,
             ring_folds,
             control = ctrl_abalone)
...
Como comparar vários modelos ao mesmo tempo?

Primeiro vamos usar `workflow_set()` e definir os argumentos:
...
Vimos os comandos `fit()` e `predict()` (+ `augment()`), mas podemos usar um atalho:

```{r}
final_fit <- last_fit(reg_wf, ring_split) 
```

pois `ring_split` tem informações sobre o conjunto de treinamento e conjunto de teste.
